...
import { attendanceApi, CheckInUrl, CheckOutUrl, empApi, GetEmployeeByEmpNoUrl } from '../constants/UrlConstans'
...
import { ResponseCheckIn } from '../interfaces/Response/ResponseCheckIn'
...
export function EmpProfilePage() {
  ...
  const { mutate: checkIn, isPending: isCheckInPending } = useMutation<ResponseCheckIn, Error, RequestCheckInOutForm>({
    mutationFn: async (payload) => {
      const { data } = await attendanceApi.post<ResponseCheckIn>(CheckInUrl, payload)
      return data
    },
    onSuccess: (data) => {
      console.log(data);
  ...
  const { mutate: checkOut, isPending: isCheckOutPending } = useMutation<ResponseCheckOut, Error, RequestCheckInOutForm>({
    mutationFn: async (payload) => {
      const { data } = await attendanceApi.patch<ResponseCheckOut>(CheckOutUrl, payload)
      return data
    },
    onSuccess: (data) => {
      console.log(data);
    ...
    <div className="flex min-h-screen bg-gray-50  gap-2 p-4">
      <div className="bg-white border border-gray-200 rounded-xl p-8 w-full max-w-sm">
        ...
        <button
          ...
          className="w-full h-10 bg-green-600 hover:bg-green-700 disabled:opacity-60
            text-white text-sm font-medium rounded-lg mt-2"
        >
          Edit Profile
        </button>
